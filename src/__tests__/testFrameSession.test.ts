...
import { REASONER_PENDING_HARD_MAX_MS } from "@/features/hse-monitoring/hooks/useReasonerHeartbeat";
...
describe("testFrameSession", () => {
  ...
  it("planTestFrameRequest reuses session id and increments frame counter", () => {
    let s = createInitialTestFrameSessionState();
    const p1 = planTestFrameRequest(s, 1000, REASONER_PENDING_HARD_MAX_MS, mint);
    ...
    s = p1.state;
    const p2 = planTestFrameRequest(s, 1100, REASONER_PENDING_HARD_MAX_MS, mint);
  ...
  it("after a pending response, the next plan is a poll (no force_reason)", () => {
    let s = createInitialTestFrameSessionState();
    s = planTestFrameRequest(s, 1000, REASONER_PENDING_HARD_MAX_MS, mint).state;
    s = applyTestFrameResponse(s, "pending", 1050);
    expect(s.pending).toBe(true);
    expect(s.pendingSinceMs).toBe(1050);
    const p2 = planTestFrameRequest(s, 1500, REASONER_PENDING_HARD_MAX_MS, mint);
  ...
  it("a terminal-success clears pending so the next plan may force again", () => {
    ...
    s = planTestFrameRequest(s, 1000, REASONER_PENDING_HARD_MAX_MS, mint).state;
    s = applyTestFrameResponse(s, "pending", 1050);
    s = planTestFrameRequest(s, 1100, REASONER_PENDING_HARD_MAX_MS, mint).state;
    ...
    const p3 = planTestFrameRequest(s, 1300, REASONER_PENDING_HARD_MAX_MS, mint);
  ...
  it("terminal-failure also clears pending", () => {
    ...
    expect(s.pending).toBe(false);
    const p = planTestFrameRequest(s, 1600, REASONER_PENDING_HARD_MAX_MS, mint);
  ...
  it("resetTestFrameSession mints a new id on the next plan", () => {
    ...
    const p = planTestFrameRequest(s, 5000, REASONER_PENDING_HARD_MAX_MS, mint);
  ...
  it("hard-max pending clears stuck-pending and allows force on the next click", () => {
    ...
    const stuckUntil = 1000 + REASONER_PENDING_HARD_MAX_MS;
    const p = planTestFrameRequest(s, stuckUntil + 1, REASONER_PENDING_HARD_MAX_MS, mint);
