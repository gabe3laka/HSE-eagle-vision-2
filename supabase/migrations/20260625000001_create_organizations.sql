-- Create tables first so the SECURITY DEFINER functions can reference them
...
-- Created AFTER tables so language sql body validation passes.
...
-- RLS policies (after functions are defined).
